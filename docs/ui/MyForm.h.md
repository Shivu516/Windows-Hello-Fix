...
## Shared hardware-toggle state (lines 9–12 in `MyForm_Camera.cpp`, `extern` here)
...
## Forward declarations (lines 42–47)
...
## The `CameraDeviceInfo` struct (lines 207–210)
...
## Free-function declarations (lines 212–222, 226)
...
- **Depended on by:** all six `MyForm_*.cpp` files (they each `#include "MyForm.h"`) and `main.cpp`.
