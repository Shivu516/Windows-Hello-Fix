...
void UpdaterPopup::InitializeComponentPopup(){
 ...
 this->ClientSize=System::Drawing::Size(360,440); this->MinimumSize=System::Drawing::Size(360,440); this->MaximumSize=System::Drawing::Size(360,520);
 ...
 // Top row: Release dropdown (68%) + Update button (30%) same Y
 cmbRelease = gcnew ComboBox();
 cmbRelease->DropDownStyle = ComboBoxStyle::DropDownList;
 cmbRelease->Location = Point(pad, y);
 cmbRelease->Size = System::Drawing::Size(228,21);
 cmbRelease->Font = gcnew Drawing::Font("Segoe UI",9);
 cmbRelease->SelectedIndexChanged += gcnew EventHandler(this, &UpdaterPopup::OnReleaseChanged);
 btnUpdate = gcnew Button();
 btnUpdate->Location = Point(pad+236, y);
 btnUpdate->Size = System::Drawing::Size(100,21);
 btnUpdate->Text = "Update";
 btnUpdate->UseVisualStyleBackColor = true;
 btnUpdate->Click += gcnew EventHandler(this, &UpdaterPopup::OnUpdateClick);
 y+=28;
 // Single preview panel
 pnlPreview = gcnew Panel(); pnlPreview->Location=Point(pad, y); pnlPreview->Size=System::Drawing::Size(336,300); pnlPreview->BackColor=SystemColors::Window; pnlPreview->BorderStyle=BorderStyle::FixedSingle; pnlPreview->Padding=System::Windows::Forms::Padding(8);
 // Inside preview: Release Info
 lblReleaseInfo = gcnew Label(); lblReleaseInfo->Location=Point(8,8); lblReleaseInfo->Size=System::Drawing::Size(320,20); lblReleaseInfo->Text="Release Info"; lblReleaseInfo->Font=gcnew Drawing::Font("Segoe UI",11,FontStyle::Bold); lblReleaseInfo->ForeColor=SystemColors::ControlText; lblReleaseInfo->AutoSize=false;
 // Status inside preview
 lblStatusInside = gcnew Label(); lblStatusInside->Location=Point(8,30); lblStatusInside->Size=System::Drawing::Size(320,16); lblStatusInside->Font=gcnew Drawing::Font("Segoe UI",9); lblStatusInside->AutoSize=false;
 // Warning inline (text only, no giant panel)
 lblWarningInline = gcnew Label(); lblWarningInline->Location=Point(8,48); lblWarningInline->Size=System::Drawing::Size(320,28); lblWarningInline->Font=gcnew Drawing::Font("Segoe UI",8); lblWarningInline->ForeColor=Color::FromArgb(152,111,11); lblWarningInline->Visible=false; lblWarningInline->AutoSize=false;
 // RichTextBox for notes
 rtbNotes = gcnew RichTextBox();
 rtbNotes->Location = Point(8,78);
 rtbNotes->Size = System::Drawing::Size(320,180);
 rtbNotes->ReadOnly = true;
 rtbNotes->ScrollBars = RichTextBoxScrollBars::Vertical;
 rtbNotes->BorderStyle = BorderStyle::None;
 rtbNotes->BackColor = SystemColors::Window;
 rtbNotes->ForeColor = SystemColors::WindowText;
 rtbNotes->WordWrap = true;
 rtbNotes->DetectUrls = false;
 rtbNotes->Font = gcnew Drawing::Font("Segoe UI",9);
 rtbNotes->LinkClicked += gcnew System::Windows::Forms::LinkClickedEventHandler(this, &UpdaterPopup::OnNotesLinkClicked);
 pnlPreview->Controls->Add(lblReleaseInfo); pnlPreview->Controls->Add(lblStatusInside); pnlPreview->Controls->Add(lblWarningInline); pnlPreview->Controls->Add(rtbNotes);
 y+=308;
 progressBar = gcnew ProgressBar(); progressBar->Location=Point(pad, y); progressBar->Size=System::Drawing::Size(336,6); progressBar->Visible=false; progressBar->Style=ProgressBarStyle::Continuous; y+=10;
 linkViewOnGithub = gcnew LinkLabel(); linkViewOnGithub->Location=Point(pad, y); linkViewOnGithub->AutoSize=true; linkViewOnGithub->Text="View on GitHub"; linkViewOnGithub->LinkColor=Color::FromArgb(99,168,248); linkViewOnGithub->VisitedLinkColor=Color::FromArgb(99,168,248); linkViewOnGithub->LinkClicked+=gcnew LinkLabelLinkClickedEventHandler(this,&UpdaterPopup::OnViewOnGithubClick);
 this->Controls->Add(cmbRelease); this->Controls->Add(btnUpdate); this->Controls->Add(pnlPreview); this->Controls->Add(progressBar); this->Controls->Add(linkViewOnGithub);
 this->ResumeLayout(false); this->PerformLayout();
}
bool UpdaterPopup::IsDarkMode() {
 try {
  ...
  if (key != nullptr) {
   ...
   if (val != nullptr) {
    int light = safe_cast<int>(val);
    return light == 0;
   }
  }
 } catch (...) {}
 // Fallback: check SystemColors::Window luminance
 try {
...
void UpdaterPopup::RefreshStatus(){
 ...
 lblReleaseInfo->ForeColor = isDark ? Color::White : Color::FromArgb(26,26,26);
 lblReleaseInfo->Font = gcnew Drawing::Font("Segoe UI", 11, FontStyle::Bold);
 if(updater==nullptr||updater->State==nullptr) return; auto st=updater->State; String^ cur=UpdateVersion::CurrentDisplayString(); GitHubRelease^ latest=st->LatestForChannel; bool isLatestAvail=st->IsUpdateAvailable();
 String^ latestTag = latest!=nullptr && latest->Version!=nullptr ? latest->Version->ToDisplayString() : (latest!=nullptr?latest->Tag:"none");
 if(isLatestAvail && latest!=nullptr){
  lblStatusInside->Text=String::Format("Current version: {0} - Latest Stable: {1}", cur, latestTag);
  lblStatusInside->ForeColor=Color::FromArgb(152,111,11); // yellow for outdated
 } else {
  lblStatusInside->Text=String::Format("Current version: {0} - Latest Stable: {1}", cur, latestTag);
  // Check if up-to-date
  if(latest!=nullptr && latest->Version!=nullptr && st->InstalledVersion!=nullptr && latest->Version->CompareTo(st->InstalledVersion)==0){
   lblStatusInside->ForeColor=Color::FromArgb(16,124,16); // green
  } else if(isLatestAvail) lblStatusInside->ForeColor=Color::FromArgb(152,111,11);
  else lblStatusInside->ForeColor=Color::FromArgb(16,124,16);
 }
 // Show/hide warning inline
 if(selectedRelease!=nullptr && !selectedRelease->HasUpdaterSupport){
  lblWarningInline->Text=String::Format("! {0} does not include the in-app updater. Future updates will require manual download from GitHub.", selectedRelease->Version!=nullptr?selectedRelease->Version->ToDisplayString():selectedRelease->Tag);
  lblWarningInline->Visible=true;
  lblWarningInline->ForeColor=Color::FromArgb(152,111,11);
 } else {
  lblWarningInline->Visible=false;
 }
 // Status for checking/downloading etc - use progressBar and lblStatusInside secondary? For now, keep lblStatusInside as version status, and use progressBar for downloading
 UpdaterStatus s=st->Status;
 if(s==UpdaterStatus::Checking){ progressBar->Visible=false; }
 else if(s==UpdaterStatus::Downloading){ progressBar->Visible=true; progressBar->Value=Math::Min(100,Math::Max(0,st->DownloadProgress)); }
 else if(s==UpdaterStatus::Installing){ progressBar->Visible=false; }
...
void UpdaterPopup::RefreshReleaseList(){
 ...
 if(list!=nullptr){ for each(GitHubRelease^ r in list){ String^ tag=r->Tag!=nullptr?r->Tag:"unknown"; String^ mark=""; if(r->Version!=nullptr&&st->InstalledVersion!=nullptr&&r->Version->CompareTo(st->InstalledVersion)==0) mark=" v installed"; String^ display=tag+mark; cmbRelease->Items->Add(display); } } cmbRelease->EndUpdate();
...
void UpdaterPopup::RefreshNotes(){
 ...
 selectedRelease=r;
 if(r==nullptr){ rtbNotes->Clear(); rtbNotes->AppendText("No release selected."); linkViewOnGithub->Visible=false; if(lblWarningInline!=nullptr) lblWarningInline->Visible=false; return; }
 linkViewOnGithub->Visible=!String::IsNullOrEmpty(r->HtmlUrl); linkViewOnGithub->Tag=r->HtmlUrl;
 // Update warning inline
 if(!r->HasUpdaterSupport){ lblWarningInline->Text=String::Format("! {0} does not include the in-app updater. Future updates will require manual download from GitHub.", r->Version!=nullptr?r->Version->ToDisplayString():r->Tag); lblWarningInline->Visible=true; } else lblWarningInline->Visible=false;
 // Adjust rtbNotes position based on warning visibility
 if(lblWarningInline->Visible){ rtbNotes->Location=Point(8,78+28); rtbNotes->Size=System::Drawing::Size(320,152); } else { rtbNotes->Location=Point(8,78); rtbNotes->Size=System::Drawing::Size(320,180); }
 // Render markdown → HTML → RichTextBox
